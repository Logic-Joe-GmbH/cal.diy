...
/**
 * Returns the org slug derived from the request hostname (e.g. `acme.cal.com` -> `acme`).
 */
export function getOrgSlug(hostname: string, forcedSlug?: string) {
  ...
  // If SINGLE_ORG_SLUG is set the instance is configured to run a single organization,
  // so we can return that slug directly regardless of the hostname.
  if (SINGLE_ORG_SLUG) {
  ...
  if (!hostname.includes(".")) {
    ...
    // A no-dot domain can never be an org domain. This treats localhost as a non-org domain.
    return null;
  }
  // Find which configured hostname is currently being used.
  ...
  // Derive the current subdomain by stripping the matched base hostname.
...
/**
 * @deprecated Use `getOrgDomainConfig` instead. It is more flexible and can be used without a request object.
 */
...
export function subdomainSuffix() {
  if (!IS_PRODUCTION && process.env.LOCAL_TESTING_DOMAIN_VERCEL) {
    // Allow testing with a valid domain so that deployment services like Vercel and Cloudflare can be tested locally.
...
export function getOrgFullOrigin(slug: string | null, options: { protocol: boolean } = { protocol: true }) {
  if (!slug) {
    // Use WEBAPP_URL if the website and webapp domains differ (e.g., EU: app.cal.eu vs cal.com).
    const useWebappUrl =
